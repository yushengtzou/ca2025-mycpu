...
class InstructionDecode extends Module {
  ...
  //   Immediate located at inst[30:20]
  //   Requires sign extension to 32 bits
  //   Hint: Use Fill() to replicate sign bit instruction(31)
  //
  val immI = Cat(
    Fill(Parameters.DataBits - 11, instruction(31)),  // Sign extension: replicate bit 31 twenty one times
    instruction(30, 25),                                // Immediate: bits [31:20]
    instruction(24, 21),
    instruction(20)
  )
  ...
  val immS = Cat(
    Fill(Parameters.DataBits - 11, instruction(31)),  // Sign extension
    instruction(30, 25),                                  // High 7 bits
    instruction(11, 8),                                   // Low 5 bits
    instruction(7)
  )
